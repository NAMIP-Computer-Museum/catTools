...
def format_localisation(localisation, id_artefact):
    ...
    if localisation is not None:
        ...
        if RE_LOCAL_NUM.match(localisation) or RE_LOCAL_PHYSIQUE.match(localisation):
            ...
            if RE_LOCAL_PHYSIQUE.match(localisation) or RE_LOCAL_NUM.match(localisation):
                config.message_info("La localisation a été corrigée automatiquement", id_artefact, localisation, avant_transformation)
                return localisation, True
...
def format_dimension(valeur, type_dimension, id_artefact):
    ...
    if bon_format.match(valeur):
        ...
        config.message_info("La " + type_dimension + " a été corrigée automatiquement", id_artefact, valeur, valeur_init)
        return valeur, True
    elif type_dimension in ("largeur", "hauteur", "longueur") and re2.match(valeur):
        split = valeur.split("m")
        valeur = float(re.sub(",", ".", split[0]))
        valeur = valeur / 10
        config.message_info("Transformation de la " + type_dimension + " en cm", id_artefact, valeur, valeur_init)
        return valeur, True
    elif type_dimension in ("poids") and re3.match(valeur):
        split = valeur.split("k")
        valeur = split[0]
        valeur = re.sub(",", ".", valeur).lstrip().rstrip()
        config.message_info("Le " + type_dimension + " a été corrigé automatiquement", id_artefact, valeur, valeur_init)
        return valeur, True
    elif type_dimension in ("poids") and re4.match(valeur):
        split = valeur.split("g")
        valeur = float(re.sub(",", ".", split[0]))
        valeur = valeur / 1000
        config.message_info("Transformation du " + type_dimension + " en kg", id_artefact, valeur, valeur_init)
        return valeur, True
    else:
        if valeur is not None and valeur not in ("None", "none"):
            ...
            config.message_avertissement(
                id_artefact, "Mauvais en encodage pour %s %s" % (determinant, type_dimension), valeur
            )
        return valeur, False
