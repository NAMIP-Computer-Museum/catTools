...
def migration(connexion, cursor):
    ...
    for ligne in tqdm(
        ...
        total=config.ligne_max - config.ligne_min + 1,
        desc="Migration Bull",
    ):
...
def artefact_request(
    ...
    sql += "'" + str(artefact["id"]) + "',"
    sql += (
        "'" + str(artefact["libelle"]) + "',"
        if artefact["libelle"] is not None
        else "NULL,"
    )
    sql += (
        "'" + str(artefact["modele"]) + "',"
    ...
    sql += (
        "'" + str(artefact["num_serie"]) + "',"
        if artefact["num_serie"] is not None
        else "NULL,"
    )
    sql += (
        ("'" + str(artefact["annee_prod"]) + "',")
        if artefact["annee_prod"] is not None
        else "NULL,"
    )
    sql += (
    ...
    sql += (
        ...
        if artefact["longueur"] is not None
        else "NULL,"
    )
    sql += (
        ("'" + str(artefact["largeur"]) + "',")
        if artefact["largeur"] is not None
        else "NULL,"
    )
    sql += (
        ("'" + str(artefact["hauteur"]) + "',")
        if artefact["hauteur"] is not None
        else "NULL,"
    )
    sql += (
        ("'" + str(artefact["poids"]) + "',")
        if artefact["poids"] is not None
        else "NULL,"
    )
    sql += (
        ("'" + str(artefact["commentaire"]) + "',")
        if artefact["commentaire"] is not None
        else "NULL,"
    )
    ...
    sql += (
        ("'" + str(id_localisation) + "',") if id_localisation is not None else "NULL,"
    )
